...
namespace EnumeratorDemo
{
    class Program
    {
        ...
        private static void DemoNonGeneric()
        {
            ...
            {
                string s = (string)enumrt.Current; // 取得目前的元素
                Console.WriteLine(s);
            }
        }
        ...
        static void DemoGeneric()
        {
            ...
            var fruits = new List<string>() { "蘋果", "香蕉", "鳳梨" };
            
            IEnumerator<string> enumrt = fruits.GetEnumerator();    // 從集合物件取得列舉器

            while (enumrt.MoveNext())   // 透過列舉器來巡訪集合中的每一個元素
            {
                string s = enumrt.Current; // 取得目前的元素
                Console.WriteLine(s);
            }
        }
